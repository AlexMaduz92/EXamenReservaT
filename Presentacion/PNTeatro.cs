...
{
    public partial class PNTeatro : Form
    {
        ...
        private void PTeatro_Load(object sender, EventArgs e)
        {
            ...
            this.dTeatroesTableAdapter.Fill(this.exaIIRADDataSet3.DTeatroes);

            int proximoId = ObtenerProximoId();
            TXTID.Text = proximoId.ToString();

            DGVTEATRO.CellDoubleClick += DGVTEATRO_CellContentClick;
        }

        private void DGVTEATRO_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = DGVTEATRO.Rows[e.RowIndex];

                TXTID.Text = row.Cells["teatroIdDataGridViewTextBoxColumn"].Value.ToString();
                TXTNOMBRE.Text = row.Cells["nombreTeatroDataGridViewTextBoxColumn"].Value.ToString();
                TXTASIENTO.Text = row.Cells["numeroAsientoDataGridViewTextBoxColumn"].Value.ToString();
            }
        }
